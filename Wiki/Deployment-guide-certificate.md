...
> **NOTE (Azure Front Door Standard):** When the deployment uses Azure Front Door, the `appDomain` output value is the AFD Standard endpoint hostname, which has the form `<baseResourceName>-<hash>.z01.azurefd.net`. The hash suffix is generated by Azure and cannot be predicted before deployment. Always copy the exact value from the deployment **Outputs** tab. If you cannot find it there, you can also read it from the Azure portal at *Front Door profile -> endpoint -> Endpoint hostname*, or via `az afd endpoint show -n <baseResourceName> --profile-name <baseResourceName> -g <resourceGroup> --query hostName -o tsv`.
...
1. Under **Manage**, click on **Authentication** to bring up authentication settings.
    ...
    1. Add a new entry to **Redirect URIs**:
        - **Type**: Web
        - **Redirect URI**: Enter `https://%appDomain%/signin-simple-end` for the URL e.g. `https://appName-<hash>.z01.azurefd.net/signin-simple-end`
...
1. Back under **Manage**, click on **Expose an API**.
    ...
    1. Click on the **Set** link next to **Application ID URI**, and change the value to `api://%appDomain%` e.g. `api://appName-<hash>.z01.azurefd.net`.
...
1. Change the `<<appDomain>>` placholder in the configurationUrl setting to be the `%appDomain%` value e.g. "`https://appName-<hash>.z01.azurefd.net/configtab`".
...
1. Change the `<<appDomain>>` placeholder in the validDomains setting to be the `%appDomain%` value e.g. "`appName-<hash>.z01.azurefd.net`".
